...
def hdps_implementation(input_df: pd.DataFrame, n: int, k: int, outcome: str, treatment: str, dimension_prefixes: list,
                        m: int = 1, threshold: Union[str, float] = '75p', outcome_cont: bool = False):
    ...
    :param outcome_cont: bool
        ...
        The HDPS implemention proposed in [1] is for binary outcome. in order to adapt the HDPS implemention for
        continous outcome too, below procedure is followed.

        When outcome D is Binary as per [1].
        RRcd = P(D =1 | C=1) / P(D=1|C = 0)

        Adaptation when outcome D is continuous.
        	Set threshold t which is 75th percentile value for the outcome column by default (but it could be set to
        median or any float or integer value if the user wishes to do so)
        	Calculate RRcd = P(D>t|C=1) / P(D>t|C=0).
        This is done by making a continuous outcome column to binary column, if value of outcome> t, then 1 else 0.
        After converting to binary, RRcd = P(D>t|C=1) / P(D>t|C=0) is equivalent to RRcd = P(D =1 | C=1) / P(D=1|C = 0).
